...
def function(inputs, outputs, updates=None, givens=None):
    ...
    Example:
        ...
        givens存放的是一个可迭代量，可以是列表，元组或者字典，即每次调用function，givens的量都会迭代变化，字典的关键字不变，但里面的值也传递到outputs的表达式z里面，用于最后计算结果
    ...
    """
    if isinstance(outputs, list):  # 判断outputs是不是list类型
...
class _Function(object):
    ...
    def __call__(self, *args, **kwargs):  # __call__把类实例变成一个可调用的对象，
        # 即_Function(inputs, outputs, updates, givens, check_nan=False)
        # 然后可调用_Function(*args, **kwargs)
        # 单星号（*）用于导入：agrs，将所有参数以元组(tuple)的形式导入，也可以用于解压参数列表
